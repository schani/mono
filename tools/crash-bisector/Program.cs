...
namespace crashbisector
{
	class BisectInfo {
		...
		bool RunWithMethods (IEnumerable<string> methods) {
			File.WriteAllLines ("/tmp/methods", methods);
			var stdout = Run ("--bisect=/tmp/methods");
			return stdout != null;
		}
	...
	class MainClass
	{
		static void UsageAndExit (int exitCode) {
			Console.Error.WriteLine ("Usage: crash-bisector.exe --mono MONO-EXECUTABLE --opt OPTION-NAME -- EXE ARG ...");
