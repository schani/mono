	...
	FOREACH_THREAD (info) {
		...
		while (start < (char**)info->client_info.stack_end) {
			if (*start >= obj && *start < endobj)
				fprintf (stderr, "Object %p referenced in thread %p (id %p) at %p, stack: %p-%p", obj, info, (gpointer)mono_thread_info_get_tid (info), start, info->client_info.stack_start, info->client_info.stack_end);
			start++;
		}
...
sgen_debug_verify_nursery (gboolean do_dump_nursery_content)
{
	...
	while (cur < end) {
		...
		if (do_dump_nursery_content) {
			...
			fprintf (stderr, "OBJ  [%p %p %d %d %s.%s %d]", cur, cur + size, (int)size, (int)ss,
					sgen_client_vtable_get_namespace (vtable), sgen_client_vtable_get_name (vtable),
					is_array_fill);
		}
