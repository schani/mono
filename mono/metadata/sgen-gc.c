...
void
sgen_parallel_pin_or_update (void **ptr, void *obj, GCVTable *vt, SgenGrayQueue *queue)
{
	for (;;) {
		mword vtable_word;
...
collect_nursery (SgenGrayQueue *unpin_queue, gboolean finish_up_concurrent_mark)
{
	...
	sgen_workers_enqueue_job (job_scan_finalizer_entries, &fin_ready_queue);
	sgen_workers_enqueue_job (job_scan_finalizer_entries, &critical_fin_queue);
