...
gint64 mono_perf_counters[MONO_PERF_STATE_NUM];
...
static void
print_jit_counters (gint64 *counters)
{
	if (mono_jit_stats.enabled) {
		g_print ("\nWallclock compilation time: %f s\n",
			counters[MONO_PERF_STATE_COMPILE] / 10000000.);
		g_print ("Wallclock execution time: %f s\n",
			counters[MONO_PERF_STATE_EXEC] / 10000000.);
	}
}
...
mono_finalize_perf_counters (MonoJitTlsData *jit_tls)
{
	...
	for (i = 0; i < MONO_PERF_STATE_NUM; i++)
		InterlockedAdd64 (&mono_perf_counters[i], jit_tls->perf_counters[i]);
...
mini_cleanup (MonoDomain *domain)
{
	...
	print_jit_counters (mono_perf_counters);
#endif
}
...
void
mono_change_perf_state_check (MonoThreadPerfState old_state, MonoThreadPerfState new_state)
{
	MonoJitTlsData *jit_tls;
	...
	jit_tls = mono_native_tls_get_value (mono_jit_tls_id);
	g_assert (jit_tls != NULL && jit_tls->state == old_state);
